		...
		for(Playlist playlist : repository.getAllPlaylists()) {
			if(name == null || playlist.getName().equals(name)) {	// Name filter
				if(isEmpty == null	// Si el filtro no se usa, incluir todas las listas
						|| (isEmpty && (playlist.getSongs() == null || playlist.getSongs().size() == 0))	// Si el filtro se pone a true, incluir las listas que no tengan canciones
						|| (!isEmpty && (playlist.getSongs() != null && playlist.getSongs().size() > 0))) {	// Si el filtro se pone a false, incluir las listas que tengan canciones
					result.add(playlist);
				}
			}
		}
